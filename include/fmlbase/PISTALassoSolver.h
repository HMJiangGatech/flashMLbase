...
namespace fmlbase{
    ...
    public:
        ...
        inline double loss_value(VectorXd *theta_t) override {
            double objval;
            objval = ((*response_vec) - (*design_mat)*(*theta_t)).squaredNorm() / sqrt(1.*ntrain_sample);
            return objval;
        }
        inline double loss_value() override {
            double objval;
            objval = ((*response_vec) - (*design_mat)*(*theta)).squaredNorm() / sqrt(1.*ntrain_sample);
            return objval;
        }
        ...
        inline double obj_value(VectorXd *theta_t) override {
            double objval;
            objval = ((*response_vec) - (*design_mat)*(*theta_t)).squaredNorm() / sqrt(1.*ntrain_sample) + lambda*(*theta_t).cwiseAbs().sum();
            return objval;
        }
        inline double obj_value() override {
            double objval;
            objval = ((*response_vec) - (*design_mat)*(*theta)).squaredNorm() / sqrt(1.*ntrain_sample) + lambda*(*theta).cwiseAbs().sum();
            return objval;
        }
        ...
        inline MatrixXd hessian() override {
            ...
            this->hessianMat = new MatrixXd(design_mat->transpose() * (*design_mat) / sqrt(1.*ntrain_sample));
            return *this->hessianMat;
        }
